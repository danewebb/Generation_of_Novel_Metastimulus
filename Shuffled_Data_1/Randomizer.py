...
class Shuffler:
    ...
    def __init__(self, x, y, dims, trsplit, tesplit):
        self.x = x
        self.y = y
        self.dims = dims
    ...
    def split(self):
        ...
        for ii in range (len(self.x)):
            ...
            if ran < self.tesplit:
                test.append(self.x[ii, :])
                test_labels.append(self.y[ii, :])
            else:
                train.append(self.x[ii, :])
                train_labels.append(self.y[ii, :])
    ...
    def rando(self):
        xx = np.ones(self.x.shape)
        yy = np.ones(self.y.shape)
        ...
        for ii in range(np.random.randint(3, 13)):
            for row in range(len(self.x)):
                r = np.random.randint(0, len(self.x))
                xx[row, :] = oldx[r, :]
                xx[r, :] = oldx[row, :]
    ...
    def train_test_split(self):
        ...
        train, train_labels, test, test_labels = self.split()
