...
class Graphing:
    ...
    def plot_loss(self, graph_dict, x, y, title):
        plt.xlabel = x
        plt.ylabel = y
        plt.title = title
    ...
    def plot_acc(self, graph_dict, x, y, title):
        plt.xlabel = x
        plt.ylabel = y
        plt.title = title
    ...
    def plot_pred_v_actual(self, prediction, actual, subx, suby, title, linestyles=[], order=[]):
        x1 = []
        y1 = []
        x2 = []
        y2 =[]
    ...
    def plot_one_loss(self, graph_dict, x, y, title, colors, linestyles = [], dicts_wanted=[], order=[], shift=[]):
        col_idx = 0
        plt.figure()
        plt.xlabel(x)
        plt.ylabel(y)
    ...
    def insertion_sort(self, sort_arr, follow_arr, sort='ascending'):
    ...
    def ordered_components(self, prediction, actual, num, color_list):
        ...
        sor, fol = self.insertion_sort(sorter, follower)
    ...
    def comparative_bar_plot(self, prediction, actual, nbars, y, title, dim=1, width=0.35, sort='ascending', plotall=False):
        # x1 = []; x2 = []
        ...
        if sort == 'ascending' or sort == 'descending':
            x2, x1 = self.insertion_sort(x2, x1, sort)
    ...
    def scatterplot3d(self, prediction, actual, type='', num_points=20, colors=[], styles=['-', 'dotted'], seed=24, savefig='untitled.pkl'):
        # todone Rico save routine
        # random.seed(seed)
        ...
        if type == '':
            ...
            cc = 0
            for ii in idxs:
                a = ax.plot([prediction[ii, 0], actual[ii, 0]], [prediction[ii, 1], actual[ii, 1]], [prediction[ii, 2], actual[ii, 2]],
                        color='k'
                        )
                b = ax.scatter3D(prediction[ii, 0], prediction[ii, 1], prediction[ii, 2], color='c', s=5)
                c = ax.scatter3D(actual[ii, 0], actual[ii, 1], actual[ii, 2], color='g', s=5)
                cc += 1
        ...
        plt.autoscale(enable=True, axis='x', tight=True)
        plt.legend((a, b, c), ('difference', 'prediction', 'actual'), loc='lower left')
        if savefig is not 'untitled.pkl':
    ...
    def multi_part(self, trloss, teloss, nuloss):
    ...
    def plot_metaloss(self, train, test, title, colors, x, y, null=None, linestyles = None, order=None, dim=0, merge=False):
    ...
    def plot_final_loss(self, train, test, title, x, y, colors, bestcount, ylim=None, null=None, linestyles = None, order=None, savepath=None, plotall=False):
        # todone Rico save routine
...
# pre-meta-optimization script
    ...
    # comparative_bar_plot(prediction, actual, nbars, y, title, dim=1, width=0.35, sort='ascending', plotall=False)
    Graphing.comparative_bar_plot(pred, act, nbars, y, title, dim=0)
    ...
    color_list = ['b', 'g', 'r', 'c', 'm', 'y', 'k']
    Graphing.scatterplot3d(pred, act, type='vector', colors=color_list, num_points=5, styles = ['-', 'dashed'], seed=3)
